...
<div class="mainright">
	...
	<div class="notice-fenlei">
		<div style="float: left; padding-right: 20px;">
			<h1>
				<span>库存商品：${sku.name}</span>
			</h1>
		</div>
		<a href="goods_skuImgAdd?skuid=${sku.id}" target="_self"><input
			class="inquire" name="" type="button" value="+添加图片"></a> <a
			href="goods_zySkuList?spuid=${sku.spuId}"><input class="inquire" name="backPage"
			type="button" value="返回库存商品列表"></a>
			...
			<tbody id="list_title">
				<c:forEach var="img" items="${imglist}" varStatus="i">
					<tr>
						<td>${i.index+1}</td>
						<td class="skuimg">
						<img width="300px" height="120px" src="<%=path %>${img.imgurl }" />
							</td>
						<td>${img.orderby }</td>
						<td>
						...
						<td><a href="goods_skuImgAdd?id=${img.id }"><span class="shenlan">编辑</span></a> 
