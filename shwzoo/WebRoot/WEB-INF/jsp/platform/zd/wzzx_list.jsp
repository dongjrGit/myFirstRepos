...
function glfl(obj){
        location.href="/platform/zd/articleAndClassify?id="+$(obj).attr("dataid")+"&title="+$(obj).attr("datatitle");
}
</script>
<div class="mainright">
    <div class="clear"></div>
        ...
        <table class="data_list">
            ...
            <script type="text/html" id="datalist">
                ...
                <tr>
                    ...
                    <td>{{art.bytitle}}</td>
                    <td>{{art._sendtime}}</td>
					<td>{{art.navclassfyName}}</td>
                    <td>{{art.clicknum}}</td>
                    <td class="zdgl-wzlbbj">
                        <input type="hidden" value="{{art.id}}" />
                        <span class="bjxx shenlan"><a href="javascript:void(0);">编辑</a></span>
						<span class="shenlan"><a href="javascript:void(0);" onclick="glfl(this)" dataid="{{art.id}}" datatitle="{{art.title}}">关联分类</a></span>
