 // 基于准备好的dom，初始化echarts实例
    ...
    var ajaxdata=AjaxUtil.Get("/zlnews/getProvinceList");
    var datalist=[];
    var datamark={};
    if(ajaxdata.code==0){
 ...
 myChart.on('click',  function eConsole(param) {
    if(VarUtil.isNotNull(param.data.code)){
	        ...
	        $("#allmap").show();
	       var ajaxdata=AjaxUtil.Post("/zlnews/getdetailByProvince",{code:param.data.code})
...
//-------------------------地图详情  传id获取数据------->
    ...
    function getMap (data_info,cityname) {  
    	...
    	// map.centerAndZoom(new BMap.Point(log, lat), 11);  // 初始化地图,设置中心点坐标和地图级别
	     ...
	     map.centerAndZoom(cityname,9);   	//定位城市中心
