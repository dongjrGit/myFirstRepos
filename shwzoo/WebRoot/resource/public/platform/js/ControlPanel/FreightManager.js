...
$(document).ready(function() {
	...
	// 地点选择框 确定/取消
	$("body").on("click",".text-inp", function() {
	...
	// 删除
	$("body").on("click",".delTemplate",  function() {
		var id = $(this).parent().parent().find("input").val();
	...
	// 地点勾选修改
	$("body").on("change",".Atareas", function() {
	...
	// 地点勾选添加
	$("body").on("change",".AreasAtrr", function() {
	...
	// 地点勾选添加
	// -----------------------------------------------------
	$("body").on("change",".AreasSelect", function() {
...
var Freight = {
	bind : function() {
		$.ajax({
			type : "post",
			url : "/platform/freightmanager/queryFreightManagerList",
			dataType : "json",
			data : {
				page : 1,
				size : 100
	...
	$.ajax({
		...
		success : function(rsl) {
			if (rsl.code == 0) {
				...
				$("input[name=IsPostage]:eq(" + tp.isexemptionpostage + ")").attr(
						"checked", 'checked');
				...
				if (tp.isexemptionpostage == 1) {
					...
					$("#pModel").val(tp.pricingmode);
					$("#sz").val(tp.num);
					$("input[name=IsConditionEdit]").eq(tp.iscondition ).attr("checked", 'checked');
					$(".num1").show();
					if(tp.pricingmode==0){
						$(".a").attr("style",null);
				    	$(".b").css("display","none");
