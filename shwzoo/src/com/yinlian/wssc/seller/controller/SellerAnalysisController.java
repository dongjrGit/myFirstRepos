 ...
 */
@RestController
@RequestMapping("/seller/analysis")
public class SellerAnalysisController {
	  ...
	  private AnalysisReportService analysisReportService;
		...
		public ReusltItem getSalesAnalysis(String starttime,String endtime,String shopid){
			...
			try{
	            ...
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询销售概况统计异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
						"查询销售概况统计异常! 异常信息:{0}", e,
						"/seller/analysis/getSalesAnalysis");
			}
			return item;
		...
		public ReusltItem SalesDetails(String starttime,String endtime,String shopid,String skuid,String page,String size){
			ReusltItem item=new ReusltItem();
			try{
				...
				//item.setData(analysisReportService.getSalesDetails(criteria));
				item.setData(analysisReportService.getSalesDetails(criteria,StringUtilsEX.ToInt(page),StringUtilsEX.ToInt(size),item));
			}
			catch(Exception e){
				item.setCode(-900);
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询销售明细异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
						"查询销售明细异常! 异常信息:{0}", e,
						"/seller/analysis/SalesDetails");
			}
			return item;
		...
		public ReusltItem getbuyerArea(String starttime,String endtime,String shopid,String page,String size){
			ReusltItem item=new ReusltItem();
			try{
				CriteriaOrder criteria=new CriteriaOrder();
				if(StringUtilsEX.ToInt(shopid)>0){
...
//				criteria.setShopid(9);
		...
		public ReusltItem getbuyerDetail(String starttime,String endtime,String shopid,String page,String size){
			ReusltItem item=new ReusltItem();
			try{
				if(StringUtilsEX.ToInt(page)<=0 || StringUtilsEX.ToInt(size)<=0){
					item.setCode(-101);
				...
				item.setCode(-900);
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询客户明细异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
		...
		public ReusltItem getCouponDetail(String starttime,String endtime,String shopid,String num,
			...
			try{
				...
				item.setCode(-900);
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询优惠卷数据异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
						"查询优惠卷数据异常! 异常信息:{0}", e,
						"/seller/analysis/getCouponDetail");
			}
			return item;
		...
		public ReusltItem getPackageDetail(String starttime,String endtime,String shopid,String num,
			...
			try{
				...
				criteria.setAddend(StringUtilsEX.ToShortDate(endtime));	
				criteria.setNum(num);
				criteria.setActivityType(ActivityHistory.组合商品.getValue());
				item.setData(analysisReportService.getPackageDetail(criteria,StringUtilsEX.ToInt(page),StringUtilsEX.ToInt(size),item));
			}
			catch(Exception e){
				item.setCode(-900);
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询组合商品数据异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
						"查询组合商品数据异常! 异常信息:{0}", e,
						"/seller/analysis/getPackageDetail");
			}
			return item;
		...
		public ReusltItem getspikeSG(String starttime,String endtime,String shopid,String num,
			...
			try{
				...
				item.setCode(-900);
	            if (DebugConfig.BLUETOOTH_DEBUG) {
	            	item.setDesc("查询闪购数据异常：" + e.toString());
				} else {
					item.setDesc("系统错误！");
				}
				LogHandle.error(LogType.analysis,
						"查询闪购数据异常! 异常信息:{0}", e,
						"/seller/analysis/getspikeSG");
			}
			return item;
