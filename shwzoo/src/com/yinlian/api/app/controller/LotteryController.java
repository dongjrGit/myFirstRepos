 ...
 * 
 * @author Liang.ma.s
 * @version $Id: LotteryController.java, v 0.1 2016年4月9日 下午1:45:07 Administrator
 *          Exp $
 */
@RestController
...
public class LotteryController {
	...
	public String lottery(String ch) {
		...
		try {
			...
			LogHandle.error(LogType.Api, "抽奖异常! 异常信息:{0}", ex, "lottery/lottery");
		}
		return item.toJson();
	}
	...
	public String getcount(String ch, String token) {
		BaseResult item = new BaseResult();
		try {
			if (!StringUtilsEX.isChannelTypeExist(ch)) {
				item.setCode(-108);
			...
			LogHandle.error(LogType.Api, "抽奖异常! 异常信息:{0}", ex, "lottery/getcount");
		}
		return item.toJson();
	}
	...
	public String getlottery(String ch) {
		BaseResult item = new BaseResult();
		try {
			if (!StringUtilsEX.isChannelTypeExist(ch)) {
				item.setCode(-108);
			...
			LogHandle.error(LogType.Api, "抽奖异常! 异常信息:{0}", ex, "lottery/getlottery");
		}
		return item.toJson();
	...
	public String showlottery(String ch, String token, Model model) {
		try {
			...
			LogHandle.error(LogType.Api, MessageFormat.format("抽奖异常! 异常信息:{0}", e.toString()), "lottery/showlottery");
		}
		return "app/lottery";
	}
	...
	public @ResponseBody String award(String ch, String token) {
		BaseResult item = new BaseResult();
		try {
			if (!StringUtilsEX.isChannelTypeExist(ch)) {
				item.setCode(-108);
			...
			LogHandle.error(LogType.Api, "抽奖异常!", e, "lottery/award");
