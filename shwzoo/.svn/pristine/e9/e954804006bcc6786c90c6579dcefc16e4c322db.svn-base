			...
			success : function(rsl) {
				if (rsl.code == 0) {
					...
					if (p.ispersent == true) {
						...
						$("#IsPersent").removeAttr("checked");
					}
					if (p.isonline == "1") {
						$("input[name='isOnline']").first().attr("checked",
								"checked");
						$("input[name='isOnline']").last()
								.removeAttr("checked");
					} else {
						$("input[name='isOnline']").last().attr("checked",
								"checked");
						$("input[name='isOnline']").first().removeAttr(
								"checked");
					}
...
function add() {
	...
	$.ajax({
		type : "post",
		url : "/platform/litteryrule/addLotteryRule",
		dataType : "json",
		data : {
			name : name,
...
function edit(id) {
	...
	$.ajax({
		type : "post",
		url : "/platform/litteryrule/updatLotteryRuleById",
		dataType : "json",
		data : {
			id : id,
			name : name,
