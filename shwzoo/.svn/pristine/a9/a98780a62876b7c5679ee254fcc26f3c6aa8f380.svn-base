    ...
    for (i  in  sFtv)
        if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
            if (Number(RegExp.$1) == (m + 1)) {
                ...
                if (RegExp.$3 == '*')  this[Number(RegExp.$2) - 1].color = '#ffffff'
            }
        ...
        if (lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
            ...
            if (tmp1 >= 0 && tmp1 < n) {
                ...
                if (tmp2 >= 0 && tmp2 < this.length) {
                    ...
                    if (RegExp.$3 == '*')  this[tmp2].color = '#ffffff'
                }
            }
        }
...
function drawCld(SY, SM) {
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            $("#SD"+i).attr("datatime",(global.currYear)+""+(global.currMonth+1<10?"0"+(global.currMonth+1):global.currMonth)+""+(sD + 1<10?"0"+(sD+1):sD+1));
           
						$("#GD" + i).unbind('click').click(function(){qiandao();});
            ...
            //(sD<todayValue-1&&todayValue!=-1)||
            if((global.currMonth==tM&&sD<todayValue-1&&todayValue!=-1)||(global.currMonth<tM)){
                $("#SD"+i).addClass("active");
            }
...
var dateSelection = {
    ...
    changeView : function() {
        ...
        yue=parseInt(global.currMonth) + 1;
    ...
    showMonthContent : function() {
        var s = '';
        for (var i = 0; i < 12; i++) {
            if ((i+1)==(parseInt(global.currMonth) + 1)){
                s+='<option id="SM' + i
                    + '" class="month" value="'+i+'" selected>' + (i + 1).toString()+'月' + '</option>';
            }else{
                s+='<option id="SM' + i
                    + '" class="month" value="'+i+'" >' + (i + 1).toString()+'月' + '</option>';
            }
        }
