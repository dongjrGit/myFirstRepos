...
var list = {
    ...
    getlist: function (index) {
        ...
        $.ajax({
            url: "/platform/businessbillsAjax/bulllist",
            type: "Post",
            data: {
                "index": index,
                "size": psize, 
...
function chuli(obj){
    ...
    location.href = "billedit?sta=" + sta+"&id="+id;
}
...
function chuli(obj){
    ...
    if (confirm("确认要处理吗？")) {
    	$.ajax({
            url: "/platform/businessbillsAjax/setBbillsStatus",
            type: "Post",
            data: { "billId": id },
            dataType: "json",
            success: function (data) {
                if (data.code == 0) {
                	...
                	location.href = "/platform/businessbills/billlist?status=0";
                } else {
                    Dalert(data.desc,10000);
                }
            }
...
function chakan(obj){
     ...
     location.href = "/platform/businessbills/bbillsOrderList?shopid="+sid+"&orderdate="+bt+"&status="+status;
}
