...
$(document).ready(function() {
	...
	$("body").on("click",".bjxx", function() {
		...
		location.href = "/platform/controlpanel/showlottery_rule_add?id=" + id;
	});
	// 添加跳转
	$("#addbtn").click(function() {
		location.href = "/platform/controlpanel/showlottery_rule_add";
	});
...
var Template = {
	bind : function(index) {
		...
		$.ajax({
			url : "/platform/litteryrule/queryLotteryAll",
			type : "post",
			data : {
				"page" : index,
				"size" : psize,
...
function del(id) {
	$.ajax({
		type : "post",
		url : "/platform/litteryrule/delLottery",
		dataType : "json",
		data : {
			id : id
		},
