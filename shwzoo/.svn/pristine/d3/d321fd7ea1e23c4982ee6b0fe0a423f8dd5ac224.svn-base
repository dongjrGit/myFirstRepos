...
<input type="hidden" id="imgsrc" value="<%=path %>" />
    ...
    <div class="account-form">
       <!--  <span>显示位置：<input type="text" id="fldisplay" class="inp-seller" /></span>
        ...
        <input id="prosptype" type="hidden" value="0"/>
        <!-- <span>
            类型：
     ...
     <input type="hidden" id="type" value="${type}">
        ...
        <span class="marrig10"></span>
        <a href="/platform/topic/tsdqProEdit?topicid=${topicid }&spmark=${spmark}&type=${type}&isby=${isby}" target="_self"><input class="chaxun" name="add" type="button" value="+添加商品"></a>
        <c:if test="${pagetype == 10 }">
        <a href="/platform/topic/showSonTopic?topicid=${topicid }&type=${type}&href=/platform/topic/tsdqprolist?pagetype=${pagetype}" target="_self"><input class="chaxun" name="add" type="button" value="子专题"></a>
        </c:if>
            ...
            <script id="SpecialProlist" type="text/html">
                ...
                <tr>
                    <td>{{if pf.type==2}}{{pf.shopname}}
							{{else}}{{if pf.type==3 || pf.type==6}}{{pf.spuname}}
                    ...
                    <td>
                        ...
                        <a href="/platform/topic/tsdqProEdit?id={{pf.id}}&topicid={{pf.topicid}}&spmark={{pf.mark}}&type={{pf.type}}&fatherid=${fatherid}"><span  class="shenlan" title="修改">修改</span></a>
