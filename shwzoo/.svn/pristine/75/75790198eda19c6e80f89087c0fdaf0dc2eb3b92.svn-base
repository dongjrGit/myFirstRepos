        ...
        if (idList != "") {
            var d = dialog({
                ...
                ok: function () {
                    ...
                    $.ajax({
                        type: "post",
                        url: "/platform/spu/delTimeStockList",
                        dataType: "json",
                        data: {
                            'ids': idList
                        },
...
var type = {
    bind: function (index) {
    	...
    	var showd=$("#showd").val();
        $.ajax({
            url: "/platform/spu/getSpuStocklist",
            type: "Post",
            data: { 
            	'showy':showy,
            	'showm':showm,
	...
	del : function() {
		...
		if (confirm('确定将此记录删除?')) {
			$.ajax({
				url : "/platform/spu/delSpuTimeStock",
				type : "Post",
				data : {
					'id' : id
				},
...
function Save() {
    ...
    $.ajax({
        url: "/platform/spu/editSpuTimeStock",
        type: "Post",
        data: {"stock":stock,
        	"price":price,
        	"id":id},
...
function backhref() {
	var spuid=$("#spuid").val();
    window.location.href = "/platform/product/showSpuStock?spuid="+spuid;
}
//刷新
function refresh() {
