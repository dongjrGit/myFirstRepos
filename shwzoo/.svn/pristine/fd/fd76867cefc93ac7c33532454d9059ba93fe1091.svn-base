...
var img = {
	...
	getimglist : function(index) {
		var id = $("#shopid").val();
		$.ajax({
			url : "/platform/shop/getImgListByCriteria",
			type : "Post",
			data : {
				"page" : index,
				"size" : psize,
			...
			success : function(data) {
				if (data.code < 0) {
					...
					Dalert("保存成功！", "", function() {
						window.location.href = 'showImageList?shopid='
								+ $("#shopid").val();
					});
				}
			},
...
function setStatus(id, ob) {
	var obtext = $("#" + ob).val();
	$.ajax({
		url : "/platform/shop/updateImgSort",
		type : "Post",
		data : {
			"id" : id,
			"sort" : obtext
