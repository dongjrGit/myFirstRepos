...
function initPage(){
	...
	if(page<=pageCount){
		var nohtml = '';
		var data=AjaxUtil.Get(pageUrl, params, AjaxDataType.JSON,beforeSend(),complete());
		if (data.code<0) {
			 alert(data.desc);
		}else{
			var listdata = {
			 ...
			 var html = template('resultList', listdata);
			 $("#"+content).html(html);
			 page = data.pageIndex;
			 pageCount=data.maxRow;
			 $("#pager").html(pagination(pageCount, pindex, psize, "list.getlist"));
		}
		if (data.code < 0) {
