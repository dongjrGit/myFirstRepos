...
<div class="mainright">
    ...
    <div class="l_wzmbtop">
        <ul>
            <li class="sj_hover"><a href="javascript:void(0)">银行卡明细</a><span class="sj-img"></span></li>
        </ul>
    ...
    <div class="dpplb">
        <table id="table_browserdetail">
            <script id="browserdetaillist" type="text/html">
                {{each list as pro i}}
                <tr>
                    <td style="vertical-align:top;">{{pro.name}}</td>
					<td>{{pro.openbank}}</td>
					<td>{{pro.subbank}}</td>
					<td>{{pro.number}}</td>
					<td>{{pro.phone}}</td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $.ajax(({
            type: "post",
            url: "/platform/membermanagement/queryBrandCard",
            dataType: "json",
