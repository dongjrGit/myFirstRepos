...
var PointsList = {
	bind : function(index) {
		var uname = $("#select_buyer").val();
		var buyername = $("#select_name").val();
		...
		$.ajax({
			type : "post",
			url : "/platform/memberfinance/getFinanceList",
			dataType : "json",
			data : {
				pageindex : index,
				pagesize : psize,
	...
	daochu:function () {
		if (confirm("确定要导出吗？")) {
			...
			location.href="/platform/member/exportfinance?type="+$("#select_type").val()+"&username="+$("#select_buyer").val()+"&" +
					"starttime="+$("#select_begin").val()+"&endtime="+$("#select_end").val()+"&" +
					"buyername="+$("#select_name").val()+"&paytype="+paytype+"&" +
