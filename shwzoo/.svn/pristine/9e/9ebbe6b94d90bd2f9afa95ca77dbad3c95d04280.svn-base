$(document).ready(function() {
    ...
    $("body").on("click", ".cxtt", function() {
        var id = $(this).parent().parent().find("input").val();
        location.href = "/seller/shop/showeditAdvert?id=" + id;
    });
        ...
        $.ajax({
            ...
            success : function(data) {
                if (data.code < 0) {
                    ...
                    $("#Advertdata").siblings().remove();
                    $("#Advertdata").after(html);
                    // 分页
                    // title = title;
                    pcount = data.maxRow;
...
function del(id) {
    $.ajax({
        type : "post",
        url : "/seller/shop/deleteAdvertById",
        dataType : "json",
        data : {
            id : id
        },
