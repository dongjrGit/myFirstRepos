...
public class ImageUtils {
	...
	public static String checkImageType(byte[] imageBytes) {
		...
		try {
			...
			while (itr.hasNext()) {
				...
				String imageName = reader.getClass().getSimpleName().toLowerCase();
				if (imageName != null && ("JPEGImageReader".toLowerCase().equalsIgnoreCase(imageName))) {
					return "jpeg";
				} else if (imageName != null && ("JPGImageReader".toLowerCase().equalsIgnoreCase(imageName))) {
					return "jpg";
				} else if (imageName != null && ("pngImageReader".toLowerCase().equalsIgnoreCase(imageName))) {
					return "png";
				} else if (imageName != null && ("gifImageReader".toLowerCase().equalsIgnoreCase(imageName))) {
					return "gif";
				}
			}
		} catch (Exception e) {
	...
	public final static byte[] scale2(byte[] file_byte, int height, int width, boolean bb) {
		try {
			double ratio = 0.0; // 缩放比例
			InputStream input = new ByteArrayInputStream(file_byte);
			...
			ImageIO.write((BufferedImage) itemp, checkImageType(file_byte), output);
	...
	public final static void pressImage(byte[] pressImg, byte[] srcImageFile, ByteArrayOutputStream destImageFile,
			int x, int y, float alpha) {
		try {
			ByteArrayInputStream iStream = new ByteArrayInputStream(srcImageFile);
			Image src = ImageIO.read(iStream);
