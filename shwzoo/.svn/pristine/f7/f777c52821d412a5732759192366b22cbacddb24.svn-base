...
public class ShowarticleServiceImpl implements showarticleService {
	...
	public int insertshowarticle(showarticle vo) throws Exception {
		...
		return showarticleMapper.insertSelective(vo);
	}
	@Override
	public PageBean getAllShow(Integer pc, Integer ps, CriteriaSku aoc)
			throws Exception {
		PageBeanUtil pageBeanUtil = new PageBeanUtil(aoc, pc, ps);// 还可以
		// 设置其他的参数
		// 多条件查询
		PageBean pageBean = pageBeanUtil.getPage();
		List<showarticle> beanList = showarticleMapper.getshowPage(pageBeanUtil);
	...
	public showarticle selectInfo(String showid) throws Exception {
		...
		return showarticleMapper.selectByPrimaryKey(StringUtilsEX.ToInt(showid));
	}
	@Override
	public PageBean getPublishList(Integer pc, Integer ps, CriteriaSku aoc)
			throws Exception {
		PageBeanUtil pageBeanUtil = new PageBeanUtil(aoc, pc, ps);// 还可以
		// 设置其他的参数
		// 多条件查询
		PageBean pageBean = pageBeanUtil.getPage();
		List<showarticle> beanList = showarticleMapper.getPublicPage(pageBeanUtil);
		List<ShowarticleDto> list=new ArrayList<ShowarticleDto>();
		for(int i=0;i<beanList.size();i++){
			ShowarticleDto dto=new ShowarticleDto();
			Integer showid=beanList.get(i).getId();
